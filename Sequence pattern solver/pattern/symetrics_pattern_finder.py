...
class SingleCentralSymmetricPatternFinder:
    def find_pattern(self, numbers, tested_central_indices=None):
        ...
        for i in range(len(numbers)):
            if numbers[i] in numbers[i+1:]:
                ...
                central_index = math.floor(statistics.mean([index, i]))

                # Vérifier que l'indice central n'a pas été testé
                if central_index in tested_central_indices:
                    continue

                # Vérifier la symétrie
                is_symmetric = self.is_symmetric_sequence(numbers, central_index)

                if is_symmetric:
                    pattern = "Symmetric"
                    next_index = len(numbers) - central_index

                    if len(numbers[:central_index]) <= len(numbers[central_index + 1:]):
                        next_element = "Complete"
                    else:
                        next_element = numbers[central_index - math.floor(statistics.mean([next_index]))]
                    return pattern, next_element
                else:
                    # Ajouter l'indice central testé et appeler récursivement find_pattern
                    new_tested_central_indices = tested_central_indices + [central_index]
    ...
    def is_symmetric_sequence(self, numbers, central_index):
        left_sequence = numbers[:central_index]
        right_sequence = numbers[central_index + 1:]
...
class DoubleCentralSymmetricPatternFinder:
    def find_pattern(self, numbers, tested_central_indices=None):
        if tested_central_indices is None:
            tested_central_indices = []
